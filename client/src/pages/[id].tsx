...
import { fetchComments, getOnePost } from '@/config/endpoints'
...
function PostPage() {
  ...
  useEffect(() => {
    async function invoke(){
      const data = await fetchComments(id,{"usertoken":localStorage.getItem("usertoken")})
      console.log(data)
  ...
  useEffect(() => {
    async function invoke(){
        const data =await getOnePost({postId:id},{"usertoken":localStorage.getItem("usertoken")})
        setPost(data.posts)
  ...
  return (
    <div className=''>
        <Head>
            <title>{post?.user?.firstname} on seeker {post?.text}</title>
        </Head>
    ...
    <div className='flex-grow border-l border-r border-gray-700 max-w-2xl sm:ml-[73px] xl:ml-[370px]'>
        <div className='flex item-center px-1.5 py-2 border-b border-r border-gray-700 text-[#d9d9d9] font-semibold text-xl gap-x-4 sticky top-0 z-50 bg-black'>
            <div className='hoverAnimation w-9 h-9 flex items-center justify-center xl:px-0'>
                <ArrowLeftIcon onClick={()=>router.back()} className='h-5 text-white'/>
            </div>
            post
        </div>{
