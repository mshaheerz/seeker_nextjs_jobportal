...
import { getAllUserDetails,getNotApprovedJobs,getAppliedJobs } from "@/config/companyendpoints";
...
function AppliedJobs() {
    ...
    useEffect(()=>{
        async function invoke(){
            const data = await getAppliedJobs({'companytoken':localStorage.getItem('companytoken')})
            console.log(data)
            setJobs(data.jobs)
        }
        invoke();
    ...
    const columns = useMemo(()=>[
        ...
        {field:'firsrname',headerName:'name',width:170, renderCell: (params) => {
            return <div className="rowitem">{params.row.user.firstname}</div>;
          }},
        {field:'lastname',headerName:'Lastname',width:170,renderCell: (params) => {
        ...
        {field:'phone',headerName:'Phone',width:150, renderCell: (params) => {
          ...
          {field:'resume',headerName:'Resume',width:150, renderCell: (params) => {
            return <div className="rowitem"><a className="text-blue-500" href={params.row.user.resume} download target="_blank">download</a></div>;
          }},
