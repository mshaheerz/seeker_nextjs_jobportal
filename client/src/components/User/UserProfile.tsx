...
function UserProfile({ users,profile,currentUserId }: any) {
 const router = useRouter()
  ...
  const updatePost = async (event: React.FormEvent<HTMLFormElement>)=>{
    ...
    if(obj.firstname && obj.lastname && obj.email && obj.city && obj.state && obj.zip &&obj.recentcompany && obj.recentjob && obj.school){
      if(regEmail.test(obj.email.toString())){
        const data = await editProfile(obj,{"usertoken":localStorage.getItem('usertoken')})
        console.log(data);
  ...
  loading ? <Loading /> :(
    ...
    <div className="h-64">
      ...
      <div className="relative bg-gray-800 w-full h-44  ">
        ...
        <img
          src={`${users?.cover?users.cover:'https://webtoolfeed.files.wordpress.com/2012/10/glasses-cat1.jpg'}`}
          alt="kitty"
          className="rounded-sm h-44 w-full object-cover mr-2"
        />
        <div className="absolute   -bottom-10 left-6 w-28 rounded-2xl bg-white h-28 ">
          <img
            src={`${users?.image?users.image:'https://www.alleycat.org/wp-content/uploads/2019/03/FELV-cat.jpg'}`}
            alt="kitty"
